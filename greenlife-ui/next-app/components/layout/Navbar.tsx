    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                'nav-link',
                pathname === link.path && 'text-primary after:w-full'
              )}
            >
              {link.name}
            </Link>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <Link href="/admin" className="p-2 text-slate-600 hover:text-primary transition-colors relative" title="Dashboard Admin">
            <ShieldCheck size={20} />
          </Link>

          <Link href="/cart" className="p-2 text-slate-600 hover:text-primary transition-colors relative">
          ...
          <div className="flex items-center gap-2">
            <Link href="/login" className="btn-secondary !px-4 !py-2 text-sm">Đăng Nhập</Link>
            <Link href="/register" className="btn-primary flex items-center gap-2 !px-4 !py-2 text-sm">
              <User size={16} />
              <span>Đăng Ký</span>
            </Link>
          </div>
          ...
          >
            <div className="flex flex-col p-6 gap-4">
              ...
              <div className="flex flex-col gap-3">
                <Link href="/cart" onClick={() => setIsMobileMenuOpen(false)} className="btn-secondary w-full text-center flex justify-center items-center gap-2">
                  Giỏ Hàng {cartCount > 0 && <span className="bg-primary text-white text-xs px-2 py-0.5 rounded-full">{cartCount}</span>}
                </Link>
                <div className="flex gap-3">
                  <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="btn-secondary flex-1 text-center">Đăng Nhập</Link>
                  <Link href="/register" onClick={() => setIsMobileMenuOpen(false)} className="btn-primary flex-1 text-center">Đăng Ký</Link>
                </div>
              </div>
            </div>
          </motion.div>
